

# Slot: isDistributionOf 


_Dataset this distribution represents_





URI: [odm:isDistributionOf](https://cdisc.org/odm2/isDistributionOf)
Alias: isDistributionOf

<!-- no inheritance hierarchy -->





## Applicable Classes

| Name | Description | Modifies Slot |
| --- | --- | --- |
| [Distribution](Distribution.md) | A technical representation that provides a specific format or access method f... |  no  |







## Properties

* Range: [Dataset](Dataset.md)





## Identifier and Mapping Information







### Schema Source


* from schema: https://cdisc.org/define-json




## Mappings

| Mapping Type | Mapped Value |
| ---  | ---  |
| self | odm:isDistributionOf |
| native | odm:isDistributionOf |




## LinkML Source

<details>
```yaml
name: isDistributionOf
description: Dataset this distribution represents
from_schema: https://cdisc.org/define-json
rank: 1000
alias: isDistributionOf
owner: Distribution
domain_of:
- Distribution
range: Dataset

```
</details>